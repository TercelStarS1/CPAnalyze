...
namespace Helpers
{
    ...
    public class DecimalHelper
    {
        public static string ToUpper(decimal d)
        {
            ...
            str1 = DecimalHelper.getupper(stry, "亿");                             //亿单元的大写  
            str2 = DecimalHelper.getupper(strw, "万");                             //万单元的大写  
            str3 = DecimalHelper.getupper(strg, "元");                             //元单元的大写  
            ...
            {
                string tmp = "";
                tmp = DecimalHelper.getint(strf[0]);
                if (tmp == "零")
                    ret = ret + tmp;
                else
                    ret = ret + tmp + "角";

                tmp = DecimalHelper.getint(strf[1]);
                if (tmp == "零")
                    ret = ret + "整";
                else
                    ret = ret + tmp + "分";
        ...
        private static string getupper(string str, string strDW)
        {
            ...
            string ret = "";
            string tmp1 = DecimalHelper.getint(str[0]);
            string tmp2 = DecimalHelper.getint(str[1]);
            string tmp3 = DecimalHelper.getint(str[2]);
            string tmp4 = DecimalHelper.getint(str[3]);
            if (tmp1 != "零")
            {
                ret = ret + tmp1 + "仟";
            }
        ...
        public static string HumanReadableFilesize(decimal size)
        {

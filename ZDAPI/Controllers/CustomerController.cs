    ...
    public class CustomerController : BasicController
    {
        ...
        public IHttpActionResult CustomerShowByPage(ZB_FEED_SALE info)   //当前页码，显示条数
        {
            using (StarOracle db = new StarOracle())
            {
                ...
                var query = db.ExecuteSqlToList<ZB_FEED_SALE>(sql).ToList();
        ...
        public IHttpActionResult CustomerShowByCYLPage(ZB_FEED_SALE info)   //当前页码，显示条数
        {
            using (StarOracle db = new StarOracle())
            {
                ...
                int startNUM = info.PageNum * info.ShowNum - info.ShowNum;
                int endNUM = startNUM + info.ShowNum;
                var sql = " select   customer, wgt from ( select customer,wgt, px, rownum rn from  (  select nvl(aaa.customer,bbb.customer) customer, nvl(aaa.wgt,0)+nvl(bbb.wgt,0) wgt, nvl(aaa.wgt,0)-nvl(bbb.wgt,0) px  from( (SELECT  S.CUSTOMER customer, sum(S.WGT) wgt  FROM zb_feed_sale S " +
                    ...
                    " ) order by px "+ srot + ")) where rn > " + startNUM + " and rn<=" + endNUM;
                var query = db.ExecuteSqlToList<ZB_FEED_SALE>(sql).ToList(); 
                ...
                var sql1 = " SELECT salesperson,customer,a2.name SalesName  FROM  ( select salesperson,customer,company from ( select ccc.*,row_number() over(partition by  customer order by eff_date desc) cn from ( " +
                     ...
                     " from zb_feed_sale bbb where company = '" + company + "' and customer  in(" + num5customer + ")) where cn = 1 )   bbb on(aaa.customer = bbb.customer and aaa.company = bbb.company and aaa.eff_date <= bbb.doc_date)  " +
                ...
                for (int i = 0; i < query.Count; i++)
                {
                    ...
                    {
                        if (query[i].CUSTOMER == result[j].CUSTOMER)
                        {
                            query[i].SalesName = result[j].SalesName;
                            break;
                        }
                    }
                }
